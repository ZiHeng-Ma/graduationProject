...
public class Apriori {
    ...
    private int SUPPORT; // 支持度阈值
    private double CONFIDENCE; // 置信度阈值
    ...
    public Apriori(List<String> transList,int SUPPORT,double CONFIDENCE) {
                ...
                for (List<String> itemList : result) {
                    if (itemList.size() < itmes.size()) {//只处理真子集
                        ...
                        for (String item : itmes)
                            if (!itemList.contains(item)) resultStr.append(ITEM_SPLIT).append(item);
